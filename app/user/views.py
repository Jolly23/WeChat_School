...
from app.weixin.checkevent import checkevent
...
@user.route('/login/<string:openid>', methods = ['GET', 'POST'])
def login(openid=None):
    ...
    if form.validate_on_submit() and openid:
        ...
        or regUser.query.filter_by(openid = openid).first():
            ...
            user = regUser(openid, form.username.data, form.password_urp.data, hashlib.md5(form.password_drcom.data).hexdigest())
            db.session.add(user)
            db.session.commit()
...
@user.route('/gradelist/<string:fromu>', methods = ['GET','POST'])
def gradeList(fromu):
    con = checkevent(fromu).key_check(str('webgrade'))
    return render_template('grade_list.html',grade=con)

@user.route('/courselist/<string:fromu>')
def courseList(fromu):
    con = checkevent(fromu).key_check(str('webcourse'))
    return render_template('course_list.html',grade=con)
